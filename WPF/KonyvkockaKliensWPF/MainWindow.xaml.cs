    ...
    public partial class MainWindow : Window
    {
        ...
        private void EditUserComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (EditUserComboBox.SelectedItem is UserDto user)
            {
                ...
                EditConfirmNewPasswordBox.Password = string.Empty;
            }
        }
        ...
        private async void UpdateUserButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                string newPassword = EditNewPasswordBox.Password;
                string confirmNewPassword = EditConfirmNewPasswordBox.Password;
                bool wantsPasswordChange = !string.IsNullOrWhiteSpace(newPassword) || !string.IsNullOrWhiteSpace(confirmNewPassword);

                if (wantsPasswordChange)
                {
                    if (string.IsNullOrWhiteSpace(newPassword) || string.IsNullOrWhiteSpace(confirmNewPassword))
                    {
                        MessageBox.Show("Jelszó módosításnál mindkét jelszó mező kitöltése kötelező.", "Figyelmeztetés", MessageBoxButton.OK, MessageBoxImage.Warning);
                        return;
                    }

                    if (!string.Equals(newPassword, confirmNewPassword, StringComparison.Ordinal))
                    {
                        MessageBox.Show("Az új jelszó és a megerősítés nem egyezik.", "Figyelmeztetés", MessageBoxButton.OK, MessageBoxImage.Warning);
                        return;
                    }

                    if (newPassword.Length < 8)
                    {
                        MessageBox.Show("Az új jelszónak legalább 8 karakter hosszúnak kell lennie.", "Figyelmeztetés", MessageBoxButton.OK, MessageBoxImage.Warning);
                        return;
                    }
                }
                ...
                UpdateAdminUserRequestDto user = new()
                {
                    ...
                    NewPasswordHash = wantsPasswordChange ? ApiService.ComputeSha256Hex(newPassword) : null,
                    NewPasswordSalt = wantsPasswordChange ? ApiService.GenerateSalt() : null
                };
