...
--
-- Gép: 127.0.0.1
-- Létrehozás ideje: 2026. Jan 29. 11:04
    ...
    IF OLD.Premium = 0 AND NEW.Premium = 1 THEN
        ...
        IF NOT EXISTS (
            ...
            VALUES (NEW.Id, 'PREMIUM_UPGRADE', 'SUSPICIOUS', 
                    CONCAT('A felhasználó prémium lett, de nem található friss vásárlás. Régi szint: ', OLD.Level));
        ELSE
            INSERT INTO `security_audit_log` (UserId, Action, Status, Details)
            VALUES (NEW.Id, 'PREMIUM_UPGRADE', 'VERIFIED', 'Legális vásárlás megerősítve.');
        END IF;
